...
namespace cv
{
static bool iCanny(const Mat& _src, Mat& _dst, float low,  float high)
{
    int size = 0, size1 = 0;
				...
				Mat &dx, Mat &dy)
{
    ...
    if( aperture_size == 3 && !L2gradient &&
        iCanny(src, dst, (float)low_thresh, (float)high_thresh) )
        return;
#endif

    const int cn = src.channels();
    dx=Mat(src.rows, src.cols, CV_16SC(cn));
    dy=Mat(src.rows, src.cols, CV_16SC(cn));

    Sobel(src, dx, CV_16S, 1, 0, aperture_size, 1, 0, cv::BORDER_REPLICATE);
    Sobel(src, dy, CV_16S, 0, 1, aperture_size, 1, 0, cv::BORDER_REPLICATE);
    //Sobel(src, dx, CV_64F, 1, 0, aperture_size, 1, 0, cv::BORDER_REPLICATE);
    //Sobel(src, dy, CV_64F, 0, 1, aperture_size, 1, 0, cv::BORDER_REPLICATE);
