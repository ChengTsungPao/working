...
def continent_filter(gray,thresh):
    ...
    for hh in range(1):
        p=mult.Pool(3)
        new_onebits=p.map(trace_base_board\
                    ,[[continents[i][0],continents_gray[i],continents[i][2]+dthresh,continents[i][1]] for i in range(len(continents))])
        p.close() 
        p.join()
        for i in range(len(new_onebits)):
            shoCV("pp",new_onebits[i])
        '''trace_base_board(old_continent[0],old_gray,new_thresh,Range)
        #===========
...
for wi in range(0,1):
    ...
    for i in range(len(continents)):
        try:
            one_blood_gray=grays[i]
            one_blood_onebit=continents[i][0]
            #showCV('pp',one_blood_onebit)
            
            #bloods.append(cell_cut(rgb,continents[i][3],continents[i][1]))
            #blood_gray.append(cell_cut(gray,continents[i][3],continents[i][1]))
            cut_gray,avg=cell_cut(gray,continents[i][3],continents[i][1])
            bloods.append(cut_gray)
            
            #showCV("oo",bloods[-1])
            #showCV("oo",blood_gray[-1])
            
            edge=cv2.Canny(one_blood_onebit, 1,1 )
            circles=cv2.HoughCircles(edge,cv2.HOUGH_GRADIENT,1,1000,param1=10,param2=1,minRadius=Rrange[0],maxRadius=Rrange[1])[0]
            
            
            circles = np.uint16(np.around(circles))
            xx=[circles[0][0],circles[0][1]]
            y=copy(xx)
            y.append(circles[0][2])
            draw_circles(todraw,[y],(255,0,0),need_center=True,draw_in_copy=False)
            if one_blood_gray[xx[1]][xx[0]]==0:
                continue
            #print(xx)
            to_find_onebit=cv2.inRange(one_blood_gray,(continents[i][2]+avg)/2,255)
            if to_find_onebit[xx[1]][xx[0]]==255:
                continue
            xx=find(to_find_onebit,xx)
            showCV("oo",to_find_onebit)
            print(xx)
            y=copy(xx)
            y.append(circles[0][2])
            draw_circles(todraw,[y],(0,255,255),need_center=True,draw_in_copy=False)
            lines,centerI=take_line(one_blood_gray,xx)
            in_r,out_r,all_r=Radiuscal(copy(todraw),lines,centerI,Pixellength,visible=False)
            inRadius.append(in_r)
            outRadius.append(out_r)
            allRadius.append(all_r)
    ...
    #print(inRadius)
    #print(outRadius)
    #path=r'C:\CodingWorkSpace\blood\newnew\tide\datas\unhealth'
    #pick=".pickle"
    #p_w=open(os.path.join(path,unhealth+"gray"+str(wi+1)+pick),'wb')    #新建或開啟 pickle檔案為寫入模式，紀錄於p_w
    #pickle.dump({'r':allRadius,'rgb':bloods},p_w) #將li 寫入p_w
    #p_w.close() #關閉
