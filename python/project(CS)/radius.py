...
from draw_and_show import showCV, draw_circles
...
def Radiuscal(todraw,bright,center,Pixellength,visible=False):
    ...
    for index in range(len(bright)):
        ...
        train_Radius.append(c*abs(ans[0]-min_center[0])*h)
        train_Radius.append(c*abs(max_right[0]-min_center[0])*h)
        train_Radius.append(c*abs(ans[1]-min_center[0])*h)
        train_Radius.append(c*abs(ans[2]-min_center[0])*h)
        train_Radius.append(c*abs(max_left[0]-min_center[0])*h)
        train_Radius.append(c*abs(ans[3]-min_center[0])*h)
    ...
    return InRadius, OutRadius, todraw, all_Radius, train_Radius
...
#-#-#-#-#-#-#-#-#main------------------------------------------
    ...
    for i in range(length):
        try:
            ...
            in_r, out_r, drawtmp, all_Radius, train_Radius = Radiuscal(copy(todraw), lines, centerI, Pixellength, visible)
            ...
            train_radius_data[path + str(index) + ".png"] = train_Radius
            train_picture_data[path + str(index) + ".png"] = np.array(continents[i][0]).flatten()/255
            index += 1
        except:
            plt.clf()
    #===================
    ...
    # print(inRadius)
    # print(outRadius)
    return inRadius, outRadius, Radius_data, train_radius_data, train_picture_data
