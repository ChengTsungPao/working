...
class augmented_reality(corner_detection):
    ...
    def draw_board(self, word):
        ...
        fs = cv2.FileStorage(self.path + "Q3_lib//" + "alphabet_lib_onboard.txt", cv2.FILE_STORAGE_READ)
    ...
    def draw_vertical(self, word):
        ...
        fs = cv2.FileStorage(self.path + "Q3_lib//" + "alphabet_lib_vertical.txt", cv2.FILE_STORAGE_READ)                
        self.draw(word.upper(), fs)
