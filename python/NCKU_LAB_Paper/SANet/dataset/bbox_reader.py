    ...
    def __getitem__(self, idx):
        ...
        if self.mode in ['train', 'val']:
            if not is_random_img:
                ...
                # Cheng Edit
                # use random
                # imgs = np.random.randn(1, self.mySize, self.mySize, self.mySize)
                ...
                # Cheng Edit
                # use image
                lines = os.listdir(os.path.join(self.data_dir, "train", "img", filename))
                lines = sorted(lines)
                slice_files = [os.path.join(self.data_dir, "train", "img", filename, s) for s in lines]
        ...
        if self.mode in ['eval']:
            ...
            # Cheng Edit
            # use random
            # image = np.random.randn(1, self.mySize, self.mySize, self.mySize)
            ...
            # Cheng Edit
            # use image
            lines = os.listdir(os.path.join(self.data_dir, "test", "img", filename))
            lines = sorted(lines)
            slice_files = [os.path.join(self.data_dir, "test", "img", filename, s) for s in lines]
            slices = [imageio.imread(s) for s in slice_files]
            imgs = np.array(slices)
