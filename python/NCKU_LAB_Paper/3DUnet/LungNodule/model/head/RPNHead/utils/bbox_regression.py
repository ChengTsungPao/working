...
def box_transform(windows, targets, weight = [1, 1, 1, 1, 1, 1]):
    """
...
def one_box_transform(windows, targets, weight = [1, 1, 1, 1, 1, 1]):
    """
    Calculate regression terms, dz, dy, dx, dd, dh, dw
    # windows should equal to # targets
    windows: [num_window, z, y, x, D, H, W]
    ...
    return np.array([dz, dy, dx, dd, dh, dw])


def box_transform_inv(windows, deltas, weight = [1, 1, 1, 1, 1, 1]):
    """
