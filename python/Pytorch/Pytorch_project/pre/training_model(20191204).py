...
def training1D(network,filename,mode):
    ...
    def get_train_data(phase):
        ...
        file = np.load((path+'/train/{},{}_train,N={},{},mu={}.npz').format(particle_data[0],particle_data[1],particle_data[2],line,particle_data[3]))
    ...
    for epoch in range(EPOCH):
        ...
        if(epoch%100==0):        
            ...
            acc.append(tmp)
        if(int(EPOCH/10)==epoch and Accuracy(test_data_all)<0.95):
            flag=0
            print("break")
            break          
    ...
    print("\n")
    print(acc)
    f = flag==1 and Accuracy(test_data_all)>0.99
    if(f):
        torch.save(dnn, time.strftime("%Y%m%d%H%M%S", time.localtime())+",phase="+str(classify_phase)+",N="+particle_data[2]+","+line+",mu="+particle_data[3]+',gpu.pkl')
    return f
...
def training2D(network,filename,mode):
    ...
    torch.save(cnn, time.strftime("%Y%m%d%H%M%S", time.localtime())+",phase="+str(classify_phase)+",N="+particle_data[2]+",mu="+particle_data[3]+',gpu.pkl')
    return True 
