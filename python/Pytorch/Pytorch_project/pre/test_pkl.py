...
def training(EPOCH,LR,BATCH_SIZE):    
    path = './data'
    classify_phase = [5,1,3,7]
    kind_of_data = ["eigenvalue","phase"]
    particle_data = ["20190823","G_matrix","8"]
    number_of_particle = int(particle_data[2])*int(particle_data[2])
    internet = (number_of_particle*2, number_of_particle, 4)
    ...
    def main():
        ...
        #print("\n")
        #print(acc)
        torch.save(dnn, "EPOCH="+str(EPOCH)+",LR="+str(LR)+",BATCH_SIZE="+str(BATCH_SIZE)+'.pkl')
...
BATCH_SIZE = [20,50,100,400,2000,4000]
for e in EPOCH:
    for l in LR:
        for b in BATCH_SIZE:
            training(e,l,b)
